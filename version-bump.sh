...
BUMP_MODE="none"
if git log -3 | grep -q "major"; then
  BUMP_MODE="major"
elif git log -3 | grep -q "minor"; then
  BUMP_MODE="minor"
else
  BUMP_MODE="patch"
fi
...
then
  ...
  mvn -q versions:set -DnewVersion="${NEW_VERSION}" -DgenerateBackupPoms=false -DprocessAllModules --file "${POMPATH}"
  git add .
